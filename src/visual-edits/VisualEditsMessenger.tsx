...
export const CHANNEL = "CHATIFY_HOVER_v1" as const;
const VISUAL_EDIT_MODE_KEY = "chatify_visual_edit_mode" as const;
const FOCUSED_ELEMENT_KEY = "chatify_focused_element" as const;
...
const postMessageDedup = (data: any) => {
  try {
    ...
    if (key === _chatifyLastMsg) return; // identical – drop
    _chatifyLastMsg = key;
  } catch {
...
// Helper to parse data-chatify-id to extract file path, line, and column
const parseChatifyId = (
  chatifyId: string
): { filePath: string; line: number; column: number } | null => {
  // Format: "filepath:line:column"
  const parts = chatifyId.split(":");
...
export default function HoverReceiver() {
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute("data-chatify-protected", "true");
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute("data-chatify-protected");
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    const chatifyId = element.getAttribute("data-chatify-id");
    if (!chatifyId) return;
    ...
    if (newText !== oldText) {
      const parsed = parseChatifyId(chatifyId);
      if (!parsed) return;
  ...
  const handleStyleChange = (
    ...
    const existingStyles = appliedStylesRef.current.get(chatifyId) || {};
    appliedStylesRef.current.set(chatifyId, { ...existingStyles, ...styles });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(chatifyId);
  ...
  const flushImageSrcChange = () => {
    ...
    const chatifyId = imgElement.getAttribute("data-chatify-id");
    if (!chatifyId) return;
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        // Extract file info from data-chatify-id
        const chatifyId = element.getAttribute("data-chatify-id");
        if (chatifyId) {
          const parsed = parseChatifyId(chatifyId);
          if (parsed) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      const hit =
        document
          ...
          ?.closest<HTMLElement>("[data-chatify-id]") ?? null;
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const hitId = hit.getAttribute("data-chatify-id");
        ...
        const tagName =
          hit.getAttribute("data-chatify-name") || hit.tagName.toLowerCase();

        // Update hover boxes immediately for instant feedback
        // Find ALL elements with the same orchids ID
        const allMatchingElements = document.querySelectorAll(
          `[data-chatify-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        allMatchingElements.forEach((element) => {
          ...
          const elementId = element.getAttribute("data-chatify-id");
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      const target = e.target as HTMLElement;
      const hit = target.closest<HTMLElement>("[data-chatify-id]");
      ...
      if (hit) {
        ...
        // Clicked on empty space or element without data-chatify-id
    ...
    function onMsg(e: MessageEvent<ParentToChild>) {
      ...
      if (e.data.msg === "CLEAR_INLINE_STYLES" && "elementId" in e.data) {
        // Find ALL elements with the same orchids ID
        const allMatchingElements = document.querySelectorAll(
          `[data-chatify-id="${e.data.elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      if (e.data.msg === "SHOW_ELEMENT_HOVER" && "elementId" in e.data) {
        ...
        if (allMatchingElements.length > 0) {
          ...
          allMatchingElements.forEach((element) => {
            ...
            if (!tagName) {
              tagName =
                element.getAttribute("data-chatify-name") ||
